...
export class BrowserSpawner {
	...
	private async findBrowserPath(
		type: "edge" | "chrome" | "firefox",
		...
		const available =
			this.context.globalState.get<IExecutable[]>(
				availableBrowserKey + type,
			) || (await this.finders[type].findAll());

		const resolved =
			runtimeExecutable === "*"
				? (available.find((r) => r.quality === "stable") ??
					available[0])
		...
		if (!resolved) {
			...
			if (
				runtimeExecutable === /* Quality.Stable */ "stable" &&
				!available.length
			) {
				throw new UserError(
					vscode.l10n.t(
	...
	private async getUserDataDir(params: ILaunchParams) {
		...
		const defaultDir = join(
			this.storagePath,
			params.browserArgs?.includes("--headless")
				? ".headless-profile"
				: ".profile",
		);
	 ...
	 */
	public async launchBrowserOnly(
		type: "edge" | "chrome" | "firefox",
		url: string,
	) {
		...
		spawn(binary, [url], {
			...
			vscode.window.showErrorMessage(
				`Error running browser: ${err.message || err.stack}`,
			);
		});
	}
	...
	public async launch(params: ILaunchParams) {
		const binary = await this.findBrowserPath(
			params.type,
			params.params.runtimeExecutable,
		);
